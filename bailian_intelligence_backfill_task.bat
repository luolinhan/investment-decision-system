...
set LOG_FILE=logs\bailian-intelligence-backfill.log
...
if errorlevel 9 (
  echo ==== [%date% %time%] Bailian intelligence backfill skipped: process active ====>> "%LOG_FILE%"
  exit /b 0
)

mkdir "%LOCK_DIR%" 2>nul
if errorlevel 1 (
  echo ==== [%date% %time%] Bailian intelligence backfill skipped: lock active ====>> "%LOG_FILE%"
  exit /b 0
)

echo ==== [%date% %time%] Bailian intelligence backfill start ====>> "%LOG_FILE%"
"%PYTHON_EXE%" scripts\translate_intelligence.py >> "%LOG_FILE%" 2>&1
set RC=%ERRORLEVEL%
echo ==== [%date% %time%] Bailian intelligence backfill end rc=%RC% ====>> "%LOG_FILE%"
